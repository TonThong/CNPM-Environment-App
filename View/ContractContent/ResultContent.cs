...
namespace Environmental_Monitoring.View.ContractContent
{
    public partial class ResultContent : UserControl
    {
        ...
        public void UpdateUIText()
        {
            ...
            if (roundedDataGridView2.Columns.Contains("TenNhanVien")) roundedDataGridView2.Columns["TenNhanVien"].HeaderText = rm.GetString("Business_Employee", culture);
        ...
        private void ResultContent_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (pi != null) pi.SetValue(roundedDataGridView2, true, null);
            }
            catch { }
        ...
        private void RoundedDataGridView2_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            using (Pen gridPen = new Pen(Color.Black, 1))
            {
                ...
                if (isMergeColumn)
                {
                    if (e.RowIndex < roundedDataGridView2.Rows.Count - 1)
                    {
                        string currVal = e.Value?.ToString();
                        string nextVal = roundedDataGridView2.Rows[e.RowIndex + 1].Cells[e.ColumnIndex].Value?.ToString();
            ...
            {
                string currVal = e.Value?.ToString();
                string prevVal = roundedDataGridView2.Rows[e.RowIndex - 1].Cells[e.ColumnIndex].Value?.ToString();
        ...
        private void LoadContract(int contractId)
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    else { row["TrangThaiHienThi"] = notApproved; motMucChuaDuyet = true; }
                ...
                {
                    if (roundedDataGridView2.Columns.Contains(col)) roundedDataGridView2.Columns[col].Visible = false;
                }
        ...
        {
            if (e.ScrollOrientation == ScrollOrientation.HorizontalScroll) roundedDataGridView2.Invalidate();
        }
            ...
            using (var form = new Requestforcorrection())
            {
                if (form.ShowDialog() == DialogResult.OK)
                {
                    ...
                    try
                    {
                        ...
                        roundedDataGridView2.DataSource = null; lbContractID.Text = rm.GetString("Plan_ContractIDLabel", culture); UpdateUIStates(false, false); this.currentContractId = 0;
                    }
